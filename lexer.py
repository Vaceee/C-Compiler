...
class Lexer():
    ...
    def parseWord(self, tuple):
        ...
        if word in self.KEY_WORD:
            ...
            if result is True:
                self.output.append([word, "NUM", word, line])
            else:
                if result != 0:
                    self.output.append([word[:result], "NUM", word[:result], line])
                    self.parseWord([word[result:], line])
        elif char.isalpha():
            result = self.recogIdentifier(word)
            if result is True:
                self.output.append([word, "ID", word, line])
                self.varCount += 1
            else:
                if result != 0:
                    self.output.append([word[:result], "ID", word[:result], line])
                    word = word[result:]
